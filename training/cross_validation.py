...
class CrossValidation:
    ...
    def train_model(self, num_epochs, batch_size, scheduler=True):
        ...
        for fold, (train_ids, test_ids) in enumerate(
            kfold.split(train_dataset, self.train_data.y_data)
        ):
            ...
            self.network = ErnNet()
            network_on_device = self.network.to(
                self.device
            ...
            for epoch in range(0, num_epochs):
                ...
                test_loss, accuracy, _, _, _ = self.evaluate_model(
                    network_on_device, testloader
                )
            ...
            # Print accuracy
            _, accuracy, _, _, _ = self.evaluate_model(
                network_on_device, testloader, show_cm=True
            )
