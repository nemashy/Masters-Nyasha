...
def train_model(
    num_epochs, train_loader, val_loader, criterion, optimizer, device, model_on_device
):
    start = time.time()
    optimizer_name, optimizer_params_dict = get_info(optimizer)
    writer = SummaryWriter(
        comment=f"_model_{model_on_device._get_name()}_criterion_{criterion._get_name()}_optimizer_{optimizer_name}"
    )
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        val_loss, val_accuracy, _, _, _ = evaluate_model(
            val_loader, device, model_on_device, criterion
        )
        ...
        early_stopping(val_loss, model_on_device)
...
def visualise_cae_performance(
    data_loader, device, model_on_device, batch_size, img_width, img_height
):
